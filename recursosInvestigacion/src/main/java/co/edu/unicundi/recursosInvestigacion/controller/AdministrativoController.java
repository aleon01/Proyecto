...
public class AdministrativoController {
	...
	public ResponseEntity<?> retornaUserPass(@PathVariable String user, @PathVariable String pass) throws ModelNotFoundException{
			administrativoDTO userUsuario = service.consultarUsuarioAndContrasena(user, pass);
			return new ResponseEntity<administrativoDTO>(userUsuario, HttpStatus.OK);			
	}
	...
	public ResponseEntity<?> retornaUserPassFind(@PathVariable String user, @PathVariable String pass) throws ModelNotFoundException{
		administrativoDTO userUsuario = service.findByAdmiUsuarioAndAdmiContrasena(user, pass);
			return new ResponseEntity<administrativoDTO>(userUsuario, HttpStatus.OK);			
	}
	
	@GetMapping("/retornar")
	public ResponseEntity<?> retornar() {
			List<administrativoDTO> listaUser = service.retornar();
			return new ResponseEntity<List<administrativoDTO>>(listaUser, HttpStatus.OK);			
	}	
	
	@GetMapping("/retornaPorId/{id}")
	public ResponseEntity<?> retornarPorId(@PathVariable int id) throws ModelNotFoundException{
		administrativoDTO user = service.retornarPorId(id);
			return new ResponseEntity<administrativoDTO>(user, HttpStatus.OK);			
	}
	
	@PostMapping("/guardar")
	public ResponseEntity<?> guardar(@Valid @RequestBody Administrativo usuario) throws ModelNotFoundException {	
			service.guardar(usuario);
			return new ResponseEntity<Object>("", HttpStatus.CREATED);				
	}
	
	@PutMapping("/editar")
	public ResponseEntity<?> editar(@Valid @RequestBody Administrativo usuario) throws ModelNotFoundException{	
