...
@RestController
@RequestMapping("/lineainvestigacion")
@CrossOrigin(origins = "*", methods= {RequestMethod.GET,RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE })
public class LineaInvController {

	@Autowired
	private ILineaInvService service;
	
	@GetMapping("/retornar")
	public ResponseEntity<?> retornar() {
			List<lineasDTO> listaLinea = service.retornar();
			return new ResponseEntity<List<lineasDTO>>(listaLinea, HttpStatus.OK);			
	}	
	
	@GetMapping("/retornaPorId/{id}")
	public ResponseEntity<?> retornarPorId(@PathVariable int id) throws ModelNotFoundException{
		lineasDTO linea = service.retornarPorId(id);
			return new ResponseEntity<lineasDTO>(linea, HttpStatus.OK);			
	}
	
	@PostMapping("/guardar")
	public ResponseEntity<?> guardar(@Valid @RequestBody LineaInvestigacion lineaInv) throws ModelNotFoundException {	
			service.guardar(lineaInv);
			return new ResponseEntity<Object>("", HttpStatus.CREATED);				
	}
	
	@PutMapping("/editar")
	public ResponseEntity<?> editar(@Valid @RequestBody LineaInvestigacion lineaInv) throws ModelNotFoundException{	
			service.editar(lineaInv);
